...
parser = argparse.ArgumentParser(
    description="Generate a Markdown table representing the CI test coverage"
)
...
class Data:
    ...
    def exists(self, container_manager, network_plugin, operating_system):
        return (
            len(
                (self.db("container_manager") == container_manager)
                & (self.db("network_plugin") == network_plugin)
                & (self.db("operating_system") == operating_system)
            )
            > 0
        )
