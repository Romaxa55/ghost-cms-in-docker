...
class SearchEC2Tags(object):
    ...
    def parse_args(self):
        ...
        parser.add_argument(
            "--list", action="store_true", default=False, help="List instances"
        )
        parser.add_argument(
            "--host",
            action="store_true",
        ...
        for group in ["kube_control_plane", "kube_node", "etcd"]:
            ...
            if cluster_name:
                filters.append(
                    {
                        "Name": "tag-key",
                        "Values": ["kubernetes.io/cluster/" + cluster_name],
                    }
                )
            ...
            for instance in instances:
                ...
                ansible_host = {"ansible_ssh_host": instance.private_ip_address}
                ...
                node_labels_tag = list(
                    filter(lambda t: t["Key"] == "kubespray-node-labels", instance.tags)
                )
                if node_labels_tag:
