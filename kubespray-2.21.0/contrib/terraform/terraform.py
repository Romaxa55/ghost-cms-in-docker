    ...
    for key, value in attributes.items():
        if isinstance(value, list):
            ...
            for i, v in enumerate(value):
                result.update(
                    convert_to_v3_structure(v, "{}{}.{}.".format(prefix, key, i))
                )
            ...
            if tf_version == 3:
                ...
                for resource in state["resources"]:
                    ...
                    for instance in resource["instances"]:
                        ...
                        data["primary"] = {
                            "attributes": convert_to_v3_structure(
                                instance["attributes"]
                            )
                        }
...
def openstack_host(resource, module_name):
    ...
    try:
        if (
            "metadata.prefer_ipv6" in raw_attrs
            and raw_attrs["metadata.prefer_ipv6"] == "1"
        ):
            attrs.update(
                {
    ...
    attrs.update({"role": attrs["metadata"].get("role", "none")})

    # add groups based on attrs
    groups.append("os_image=" + str(attrs["image"]["id"]))
    groups.append("os_flavor=" + str(attrs["flavor"]["name"]))
    groups.extend(
        "os_metadata_%s=%s" % item for item in list(attrs["metadata"].items())
    )
...
def iter_host_ips(hosts, ips):
    ...
    for host in hosts:
        ...
        if (
            "use_access_ip" in host[1]["metadata"]
            and host[1]["metadata"]["use_access_ip"] == "0"
        ):
