...
ROLES = ["all", "kube_control_plane", "kube_node", "etcd", "k8s_cluster", "calico_rr"]
...
KUBE_CONTROL_HOSTS = int(
    os.environ.get("KUBE_CONTROL_HOSTS", os.environ.get("KUBE_MASTERS", 2))
)
...
class KubesprayInventory(object):
    ...
    def write_config(self, config_file):
        if config_file:
            ...
            print(
                "WARNING: Unable to save config. Make sure you set "
                "CONFIG_FILE env var."
            )
            ...
            if group == "all":
                ...
                if group not in self.yaml_config:
                    all_dict = OrderedDict(
                        [("hosts", OrderedDict({})), ("children", OrderedDict({}))]
                    )
        ...
        for host in changed_hosts:
            ...
            # then we assume its an IP address
                ...
                if USE_REAL_HOSTNAME:
                    cmd = (
                        "ssh -oStrictHostKeyChecking=no " + access_ip + " 'hostname -s'"
                    )
    ...
    def purge_invalid_hosts(self, hostnames, protected_names=[]):
        for role in self.yaml_config["all"]["children"]:
            if (
                role != "k8s_cluster"
                and self.yaml_config["all"]["children"][role]["hosts"]
            ):  # noqa
                all_hosts = self.yaml_config["all"]["children"][role][
                    "hosts"
                ].copy()  # noqa
                for host in all_hosts.keys():
                    if host not in hostnames and host not in protected_names:
                        self.debug(
                            "Host {0} removed from role {1}".format(host, role)
                        )  # noqa
                        del self.yaml_config["all"]["children"][role]["hosts"][
                            host
                        ]  # noqa
        # purge from all
    ...
    def set_calico_rr(self, hosts):
        for host in hosts:
            if (
                host in self.yaml_config["all"]["children"]["kube_control_plane"]
            ):  # noqa
                self.debug(
                    "Not adding {0} to calico_rr group because it "
        ...
        for host in hosts:
            if len(self.yaml_config["all"]["hosts"]) >= SCALE_THRESHOLD:
                if host in self.yaml_config["all"]["children"]["etcd"]["hosts"]:  # noqa
                    self.debug(
                        "Not adding {0} to kube_node group because of "
