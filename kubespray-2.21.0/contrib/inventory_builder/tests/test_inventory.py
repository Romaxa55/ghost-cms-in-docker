...
class TestInventory(unittest.TestCase):
    ...
    def test_get_ip_from_opts_invalid(self):
        ...
        self.assertRaisesRegex(
            ValueError, "IP parameter not found", self.inv.get_ip_from_opts, optstring
        )
    ...
    def test_get_host_id_invalid(self):
        ...
        for hostname in bad_hostnames:
            self.assertRaisesRegex(
                ValueError, "Host name must end in an", self.inv.get_host_id, hostname
            )
    ...
    def test_purge_invalid_hosts(self):
        ...
        self.assertNotIn(bad_host, self.inv.yaml_config["all"]["hosts"].keys())
    ...
    def test_add_host_to_group(self):
        ...
        self.assertEqual(
            self.inv.yaml_config["all"]["children"][group]["hosts"].get(host), None
        )
    ...
    def test_set_kube_control_plane(self):
        ...
        self.assertIn(host, self.inv.yaml_config["all"]["children"][group]["hosts"])

    def test_set_all(self):
        hosts = OrderedDict([("node1", "opt1"), ("node2", "opt2")])

        self.inv.set_all(hosts)
        for host, opt in hosts.items():
            self.assertEqual(self.inv.yaml_config["all"]["hosts"].get(host), opt)
    ...
    def test_set_k8s_cluster(self):
        ...
        for host in expected_hosts:
            self.assertIn(
                host, self.inv.yaml_config["all"]["children"][group]["children"]
            )
    ...
    def test_scale_scenario_one(self):
        ...
        for h in range(3):
            self.assertFalse(
                list(hosts.keys())[h]
                in self.inv.yaml_config["all"]["children"]["kube_node"]["hosts"]
            )
    ...
    def test_range2ips_incorrect_range(self):
        ...
        self.assertRaisesRegex(
            Exception,
            "Range of ip_addresses isn't valid",
            self.inv.range2ips,
            host_range,
        )
