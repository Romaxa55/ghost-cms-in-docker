...
parser = argparse.ArgumentParser(description="Cleanup old branches in a GitLab project")
parser.add_argument(
    "--api",
...
parser.add_argument(
    "--age", type=int, default=30, help="Delete branches older than this many days"
)
parser.add_argument(
    "--prefix",
    default="pr-",
...
parser.add_argument("project", help="Path of the GitLab project")
...
for b in p.branches.list(all=True):
    ...
    if (
        ...
        and not b.protected
        and not b.default
        and b.name.startswith(args.prefix)
    ):
        print("Deleting branch %s from %s ..." % (b.name, date.date().isoformat()))
        if not args.dry_run:
            b.delete()
